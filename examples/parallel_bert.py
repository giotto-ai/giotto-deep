...
def download_dataset():
    if not Path("cola_public").exists():
...
def main(args):
    ...
    tmp_path = Path("./cola_public") / "raw" / "in_domain_train.tsv"
    df = pd.read_csv(
        tmp_path,
        delimiter="\t",
        header=None,
    ...
    train_inputs, validation_inputs, train_labels, validation_labels = train_test_split(
        input_ids, labels, random_state=13, test_size=0.1
    )

    dl_builder = DataLoaderBuilder(
        (
            FromArray(train_inputs, train_labels),
            FromArray(validation_inputs, validation_labels),
        )
    )

    dl_tr, dl_val, _ = dl_builder.build(
    ...
    parallel = Parallelism(
        args.parallel, devices, len(devices), config_fsdp=config_fsdp, pipeline_chunks=2
    )
