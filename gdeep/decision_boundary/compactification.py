...
class Compactification():
    ...
    def __init__(self,precision = 0.4,
        ...
        Args:
            ...
            neural_net: nn.Module
                ...
                the boundary
            boundary_tuple: list of (left,right). Required
        ...
        if i == -1:
            ...
            for dim in range(sample_points_tensor.shape[1]):
                if dim == i:
                    ...
                    stacking_list.append(sample_points_tensor[:,dim]/sample_points_tensor[:,i])
    ...
    def create_final_distance_matrix(self):
        ...
        if self.check_compute_charts == False:
            self.compute_charts()
        ...
        for i in range(len(self.patches)):
            ...
            for j,pts in enumerate(self.patches):
                if j == i:
                    ...
                    list_of_pts_per_patch.append(self.transition_to_patch(self.transition_to_patch(pts,j-1),i-1))
    ...
    def plot_chart(self,i):
        ...
        '''
        df_plot = pd.DataFrame(self.list_of_pts_in_patches[i],columns = ["x" + str(j) for j in range(self.list_of_pts_in_patches[i].shape[1])])
