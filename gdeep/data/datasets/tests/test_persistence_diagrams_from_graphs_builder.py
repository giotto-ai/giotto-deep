...
def test_mutag_load_and_save():
    ...
    gudhi_ext = (
        ...
        np.array(
            [
                [0.90659795, 0.90710085],
                [0.90672356, 0.90710077],
                [0.90672356, 0.90684934],
            ]
        ),
        np.array([[0.90659781, 0.90710077], [0.90659781, 0.90710085]]),
    )
    ...
    file_path: str = os.path.join(
        DEFAULT_GRAPH_DIR,
        f"MUTAG_{diffusion_parameter}_extended_persistence",
        "diagrams",
    )
    graph_idx = 1
    pd = OneHotEncodedPersistenceDiagram.load(
