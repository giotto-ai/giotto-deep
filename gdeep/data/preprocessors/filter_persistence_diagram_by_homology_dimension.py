...
class FilterPersistenceDiagramByHomologyDimension(
    ...
    def __init__(self, homology_dimensions_to_filter: List[int]):
    ...
    def __call__(self, item: Tuple[PD, T]) -> Tuple[PD, T]:
        ...
        if not self.is_fitted:
            raise RuntimeError(
                "The filter is not fitted to any dataset. "
        ...
        for homology_dimension in self.homology_dimensions_to_filter_by:
            out.append(
                item[0]
                .get_all_points_in_homology_dimension(homology_dimension)
                .get_raw_data()
            )
