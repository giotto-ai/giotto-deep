...
class IntersectionHomology(BaseEstimator, TransformerMixin):
    ...
    Args:
        - knn (int) : number of nearest nneigbours

    """
    def __init__(self, knn: int = 5, perversity=None,
                 perversity_check=True, coarse_strata=False):
        self.knn = knn
        self.gg = None
    ...
    def fit(self, X):
        ...
        if type(X) == np.ndarray or type(X) == list:
            ...
            self.gg = X
        self.simplices, self.maximal_cliques = \
            self.from_graph_to_complex(self.gg)
    ...
    def transform(self, X, phi_list_proper=None):
        if self.check_is_fitted:
            if phi_list_proper is None:
                if self.perversity_check:
                    return self.phi_generators(self.simplices,
                                               self.list_proper)
    ...
    def _reduce(D1, verbose: bool = False):
        ...
        for j in range(D1.shape[1]):
            ...
            while j1 < j:
                # print(j,j1)
                if IntersectionHomology._low_(D1, j1) == \
                    IntersectionHomology._low_(D1, j) and \
                        IntersectionHomology._low_(D1, j) != 0:
    ...
    def _compute_homology(self, D1, proper_list=None):
        ...
        for index in index_positive_simplices:
            if not np.array([True if index in pair else False for
                             pair in index_pairings]).any():
    ...
    def phi(self, D, lista: list):
        '''inputs the incidence matrix and outputs the m x s
        incidence matrix,
    ...
    def from_graph_to_complex(self, G):
        ...
        max_cliques = list(map(list,
                           list(map(set, list(find_cliques(G))))))
        map(list.sort, max_cliques)
        # simplices_dupl = list(map(list,np.array(G.nodes).reshape(-1,1))) + \
        #     list(map(list,list(G.edges))) + cliques
        # simplices = []
    ...
    def _boundary_matrix(self, simplices):
        ...
        boundary_matrix = csr_matrix((data, (row, col)),
                                     shape=(length,
                                            length)).toarray()
    ...
    def homology_array(self, simplices):
        ...
        homology_generators = np.array(list(map(len,
                                                simplices)))[comp_hom] - 1
    ...
    def generators(self, simplices):
        ...
        for index in range(len(arr)):
            output.append((arr[index],
                           list(np.array(simplices,
                                         dtype=object)[(np.nonzero(vv[index])[0]
                                                       )])))
        return output
    ...
    def check_stratum(self, sigma, maximal_cliques):
        '''Define a stratum to be a set of simplices such that each simplex
            in one stratum is:
            1. it is a maximal d-dimensional clique
            2. OR it is a d-dimensional simplex such that it belogs to
                3 or more max simplices that
                are not otherwise connected. If they are
                otherwise connected, the graph of cthe max
                simplices connectivity is computed and analysed.
                If there are more than one connected components,
                then we have a d-dimensional piece of a stratum
            3. OR it belongs to exactly 2 max cliques of dimension
                larger than d+2
        '''
        cond1 = sigma in maximal_cliques
        num_inter = np.sum([set(sigma).issubset(set(max_spx))
                            for max_spx in maximal_cliques])
        cond2 = False
        if num_inter > 2:
            intersecting_max_spxs = [max_spx for max_spx in maximal_cliques
                                     if set(sigma).issubset(set(max_spx))]
            G_new = nx.Graph()
            for s in intersecting_max_spxs:
                proto_edges = np.nonzero([len(set(s).intersection(ss)) >
                                          len(sigma) for ss
                                          in intersecting_max_spxs])[0]
        ...
        cond3 = False
        if num_inter == 2:
            cond3 = np.sum([set(sigma).issubset(set(max_spx)) *
                           (len(max_spx) - len(sigma)) > 1 for max_spx
                           in maximal_cliques]) == 2
        return cond1 or cond2 or cond3

    def perv_list(self, perversity):
        '''This function produces the list of
        proper simplices, to be
        inputted directly to the `fit` method.
        '''
        n=max(map(len, self.simplices))
        # there is the option to get the coarse
        # stratification with the next line.
        ...
        for i, sigma in enumerate(self.simplices):
            ...
            for k in range(n):
                ...
                stratum = [s for s in self.simplices if len(s) == n-k
                           and self.check_stratum(s, self.maximal_cliques)]
                # print(stratum, sigma)
                if self.coarse_strata:
                    stratum = strata[n-k-1]
                else:
                    stratum = [s for s in self.simplices if len(s) == n-k
                               and self.check_stratum(s,
                                                      self.maximal_cliques)]
                # print(stratum, sigma)
                if len(stratum)>0:
                    intersection_temp = \
                        [set(sigma).intersection(set(simplex_stratum))
                         for simplex_stratum in stratum]
...
def _powerset(iterable):
    ...
    s = list(iterable)
    return list(map(list, chain.from_iterable(combinations(s, r)
