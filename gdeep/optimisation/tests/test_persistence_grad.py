...
def test_PersistenceGradient_matrix():
    ...
    pg = PersistenceGradient(homology_dimensions=(0,1),zeta=0.0,collapse_edges=False,
                             metric="precomputed")
...
def test_PersistenceGradient_matrix_2():
    dist=torch.tensor([[ 0.,  2., 10., 10.],
        [ 2. ,  0.,  2.,   1],
        [10.,  2. ,  0.,  1],
        [10.,  1 ,  1,  0.]]) # simulate the weighted graph

    pg = PersistenceGradient(homology_dimensions=(0,1),zeta=0.0,
                             collapse_edges=False,
                             metric="precomputed")
    assert all(pg.phi(dist) == torch.tensor([ 0.,  0.,  0.,  0.,  1.,  1.,  2.,  2.,  2., 10., 10., 10., 10., 10.]))
    assert pg.persistence_function(dist).item() == -4.
    fig, fig3d, loss_val = pg.SGD(dist,n_epochs=1,
                             lr=0.002)
